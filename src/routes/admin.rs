        ...
        .map_err(|e| AppError(e.to_string()))?
    {
        ...
        match name.as_str() {
            "image" => {
                ...
                let ext = match filename.rsplit_once('.').map(|(_, e)| e) {
                ...
                input.image_url = upload_image(state, &data, ext).await?;
            }
            _ => {
...
/// Uploads to the public `products` bucket in Supabase Storage, returns the public URL.
async fn upload_image(state: &AppState, data: &[u8], ext: &str) -> Result<String, AppError> {
    if state.cfg.supabase_url.is_empty() {
        return Err(AppError(
            "SUPABASE_URL not configured for image uploads".into(),
        ));
    }
    let filename = format!("{}.{ext}", Uuid::new_v4());
    let url = format!(
        "{}/storage/v1/object/products/{filename}",
    ...
    let res = state
        .http
        .post(&url)
        .bearer_auth(&state.cfg.supabase_service_role_key)
    ...
    if !res.status().is_success() {
        ...
        return Err(AppError(format!("storage upload failed: {body}")));
    }
    Ok(format!(
        "{}/storage/v1/object/public/products/{filename}",
        state.cfg.supabase_url
    ))
}
