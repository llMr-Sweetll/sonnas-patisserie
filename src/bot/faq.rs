...
pub fn match_faq(text: &str) -> Option<String> {
    ...
    if has(&["allerg", "nut", "gluten", "dairy", "healthy", "bromate", "preservative", "additive", "maida", "vanaspati"]) {
    ...
    if has(&["custom", "personalis", "personaliz", "message on", "name on", "write on", "design", "photo cake", "theme"]) {
    ...
    if has(&["deliver", "delivery area", "where do you", "come to", "shipping", "outside hubli"]) {
    ...
    if has(&["pay", "payment", "upi", "card", "cash", "razorpay", "gpay", "online"]) {
    ...
    if has(&["how long", "lead time", "advance", "same day", "today", "tomorrow", "notice"]) {
...
mod tests {
    ...
    fn faq_matches_common_questions() {
        assert!(match_faq("do you have eggless cakes?").unwrap().contains("eggless"));
        assert!(match_faq("what time do you open").unwrap().contains("2pm"));
        assert!(match_faq("where are you located").unwrap().contains("Hubli"));
